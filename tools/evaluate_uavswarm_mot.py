#!/usr/bin/env python3
"""Evaluate UAVSwarm MOT results with motmetrics and TrackEval HOTA."""
...
def iou_matrix(gt_boxes, tracker_boxes):
    ...
    tracker_right, tracker_bottom = tracker[:, 0] + tracker[:, 2], tracker[:, 1] + tracker[:, 3]
    intersection = (np.maximum(0.0, np.minimum(gt_right[:, None], tracker_right[None, :]) -
                               np.maximum(gt[:, None, 0], tracker[None, :, 0])) *
                    np.maximum(0.0, np.minimum(gt_bottom[:, None], tracker_bottom[None, :]) -
...
def serialise_mot_row(row):
    ...
    for field in MOT_FIELDS:
        ...
        if not np.isfinite(value):
            serialised[field] = None
        elif field in COUNT_FIELDS:
            serialised[field] = int(value)
        elif field in RATE_FIELDS:
            serialised[field] = round(100.0 * value, 6)
        else:
            serialised[field] = round(value, 6)
    return serialised
...
def main():
    ...
    for sequence_dir in sequence_dirs:
        ...
        if not tracker_path.is_file():
            raise FileNotFoundError('missing tracker result ' + str(tracker_path))
        ...
        accumulators.append(mm.utils.compare_to_groundtruth(
            gt, tracker, 'iou', distth=args.iou_threshold))
        names.append(name)
    ...
    output = {
        ...
        'overall': {'motmetrics': serialise_mot_row(mot_summary.loc['OVERALL']),
                    'hota': serialise_hota(hota_overall, hota_metric)},
